...
let WHISPER_RE = /:(\w+)!\w+@\w+\.tmi\.twitch\.tv WHISPER #(\w+) :(.+)/;
let JOIN_PART_RE = /:(\w+)!\w+@\w+\.tmi\.twitch\.tv (JOIN|PART) #(\w+)/;
let MODE_RE = /:jtv MODE #(\w+) (\+|\-)o (\w+)/;
let HOSTTARGET_RE = /:tmi\.twitch\.tv HOSTTARGET #(\w+) :(\w+|-) (\d+|-)/;
let NAMES_LIST_RE = /:\w+\.tmi\.twitch\.tv 353 (\w+) = #(\w+) :(.+)/;
let CONNECTED_RE = /:tmi\.twitch\.tv 376 (\w+) :>/;
let USER_ROOM_STATE_RE = /:tmi\.twitch\.tv (USERSTATE|ROOMSTATE) #(\w+)/;
